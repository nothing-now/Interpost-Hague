...
/datum/config_entry/proc/can_vv_get(var_name)
	. = ..()
	if(var_name == NAMEOF(src, config_entry_value) || var_name == NAMEOF(src, default))
		. &= !(protection & CONFIG_ENTRY_HIDDEN)

/datum/config_entry/proc/vv_edit_var(var_name, var_value)
...
/datum/config_entry/number/ValidateAndSet(str_val)
	...
	if(!isnull(temp))
		...
		if(config_entry_value != temp && !(datum_flags & DF_VAR_EDITED))
			log_world("Changing [name] from [temp] to [config_entry_value]!")
		return TRUE
	return FALSE
