...
/obj/machinery/media/jukebox/proc/StartPlaying()
	...
	// It also has the "ignore_vis" flag so it can be heard through walls and zlevels...but I changed that, because that's kinda gay.
	sound_token = GLOB.sound_player.PlayLoopingSound(src, sound_id, current_track.sound, volume = volume, range = 10, falloff = 3, prefer_mute = TRUE, ignore_vis = FALSE, streaming = TRUE)
	playing = 1
	update_use_power(2)
	update_icon()
