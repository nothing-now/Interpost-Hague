	...
	for(var/datum/surgery_step/S in surgery_steps)
		...
		if(S.tool_quality(src))
			...
			if(step_is_valid && S.is_valid_target(M))
				...
				if(user.skillcheck(user.skills["medical"], 65, null, "Medical") || user.statcheck(user.stats["int"], 8 ,"I don't have enough training for this...  Maybe if I keep trying I can get it.", "Int"))
